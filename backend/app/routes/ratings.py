...
def rate_book():
    ...
    rating_entry = BookRating.query.filter_by(
        user_id=user_id, book_isbn=book_id
    ).first()
    if rating_entry:
...
def rate_user():
    ...
    exchange = BookExchange.query.filter_by(
        user_id=rater_id, owner_id=rated_id, status=3
    ).first()
    if not exchange:
        return (
            jsonify(
                {"error": "You need to complete a transaction with the user first."}
            ),
            403,
        )

    rating_entry = UserRating.query.filter_by(
...
def get_book_ratings(book_id):
    ...
    result = [
        {"user_id": rating.user_id, "rating": rating.rating} for rating in ratings
    ]
...
def get_book_ratings_by_user(user_id):
    ...
    result = [
        {"book_id": rating.book_isbn, "rating": rating.rating} for rating in ratings
    ]
    return jsonify(result), 200
