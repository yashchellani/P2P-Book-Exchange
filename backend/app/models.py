...
class UserRating(db.Model):
    ...
    rater = db.relationship(
        ...
        backref=db.backref("rater_ratings", lazy="dynamic"),
    )
    ratee = db.relationship(
        "User",
        foreign_keys=[ratee_id],
        backref=db.backref("ratee_ratings", lazy="dynamic"),
    )
...
class BookExchange(db.Model):
    ...
    status = db.Column(
        db.Enum(BookExchangeStatus), nullable=False, default=BookExchangeStatus.PENDING
    )
    book_isbn = db.Column(db.String(120), db.ForeignKey("book.isbn"), primary_key=True)
    user = db.relationship(
        "User", foreign_keys=[seller], backref=db.backref("book_seller", lazy="dynamic")
    )
